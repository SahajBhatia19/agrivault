...
import { ArrowLeft, Camera, Thermometer, ShieldAlert, FileText, CheckCircle2, AlertTriangle, QrCode } from 'lucide-react';

export default function CanonicalBatchDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <div className="flex-1 flex max-w-7xl w-full mx-auto pb-20 md:pb-6">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 overflow-x-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-400">
                  DYNAMIC BATCH IDENTITY & QR PASSPORT
                </span>
                <h1 className="text-xl font-extrabold text-white tracking-tight">
                  Batch Detail & Passport ({batchId})
                </h1>
              </div>
            </div>
          ...
          {loading ? (
            <div className="p-12 text-center text-slate-500 text-sm">Loading dynamic batch data from PostgreSQL backend...</div>
          ) : batch ? (
